...
## Week 2 in progress (2026-07-30) — steps 1–7 of 14 done
...
- **Out of band (2026-07-30) — `fix(api): stop leaking internal error detail in 500 responses`** (`81ce6a8`, committed). Not a plan step; found while walking the architecture. `NewStrictHandler(server, nil)` and `HandlerFromMux` both accept the generated default error handlers, which write `err.Error()` into the body as `text/plain`. A failing query therefore handed the caller the table name, constraint name and SQLSTATE, and *every* error response violated the `Error` schema the spec promises and the generated TS client parses. New `internal/api/errors.go` supplies `StrictOptions(logger)` (both strict hooks) and `ChiErrorHandler` (the param-binding hook, which the strict layer never sees — **there are three hooks, not one**). Policy: a 4xx is the caller's fault and safe to describe, since those messages contain only spec-declared parameter names and the caller's own input; a 5xx is opaque, with the detail logged and the request id echoed so a report can be traced. `errors_test.go` pins both directions — the response must not contain the error, the log must.

- **Step 7 — auth wired into the backend.** `/api/v1` now requires a credential; `/healthz` stays public.
  - `api.Server` widened by **variadic options** (`WithTxBeginner`, `WithLogger`), so the existing `NewServer(nil)` call sites compiled untouched. `TxBeginner` is an interface (`Begin(ctx) (pgx.Tx, error)`), not `*pgxpool.Pool`.
  - Mutations open with a `dbReady()` guard → **503 `database_unavailable`** when there is no database, so no-DB mode survives. `TestMutationsReturnNotImplemented` became `TestMutationsWithoutDBReturnUnavailable`, plus a new `TestMutationsWithDBReturnNotImplemented` proving the 501 stubs are still reachable once a database is present.
  - `GetCurrentUser` implemented from the principal, no query of its own, roles sorted by team name for a stable response. Two tests cover it.
  - **New `internal/app/router.go`** — `NewRouter(ctx, Deps) (http.Handler, error)` builds the production middleware chain. It returns an error because Entra mode performs OIDC discovery there. The authenticator mounts on the `/api/v1` sub-router **only when `Deps.Queries != nil`**, since the resolver cannot work without a database. This package exists so step 10's integration test exercises the real router.
  - `main.go` shrank to config → migrate → pool → seed → `app.NewRouter` → serve. The pool is hoisted so it can reach both the seed and the router.

  Verified: `go build ./...`, `go tool golangci-lint run` (0 issues), `go test -short ./...`. All five of the plan's curl checks pass against the live stack (`/healthz` 200 public, unauthenticated `/api/v1/services` 401, `dev.editor` 200, `/me` returns both asymmetric roles sorted, unknown email 401). No-DB mode checked natively: read 200 empty, mutation 503, `/me` 401. Playwright: signed out `/services` → `/signin`; signed in as Dev Editor, all 5 services render **through the BFF with the backend now enforcing auth**, and `/api/v1/me` through the proxy returns the roles.
...
- **A nil `*pgxpool.Pool` assigned into an interface field is not nil** (found in step 7). `app.Deps.Tx` is an `api.TxBeginner`; passing a nil pool straight in produces a non-nil interface holding a nil pointer, so `dbReady()`'s `s.tx != nil` check would pass and the mutation would panic instead of answering 503. Fix in place: `txBeginner(pool)` in `main.go` reintroduces the nil at the conversion. Any future `Deps` field taking a possibly-nil pointer as an interface needs the same treatment.
- **`NewStrictHandler(x, nil)` and `HandlerFromMux` silently accept leaky error handlers** (fixed 2026-07-30, `81ce6a8`). The generated defaults write `err.Error()` as `text/plain`, so a 500 leaks database internals and every error response breaks the `Error` schema contract. Use `NewStrictHandlerWithOptions` + `HandlerWithOptions` with `api.StrictOptions(logger)` and `api.ChiErrorHandler`. **Three hooks exist, at three depths**: `ChiServerOptions.ErrorHandlerFunc` (param binding), `StrictHTTPServerOptions.RequestErrorHandlerFunc` (body decode), `...ResponseErrorHandlerFunc` (handler returned an error). Wiring only the strict pair leaves the param path leaking. If a regeneration ever resets this, `errors_test.go` fails.
- **`git checkout HEAD -- <file>` writes CRLF on this machine, and gofmt rejects CRLF** (found in step 7 while verifying the commit split). `core.autocrlf=true`, so restoring a file to verify an intermediate commit state makes golangci-lint fail with `File is not properly formatted (gofmt)` on a file nobody touched. It is an artifact of the checkout, not of the committed code (git stores LF; CI on ubuntu is unaffected). Fix while verifying: `sed -i 's/\r$//' <file>` after the checkout. Better: back the changed files up to the scratchpad and restore by `cp` rather than `git checkout`.
- **Playwright MCP screenshots do not always land in `.playwright-mcp/`.** A `filename` given as a bare name wrote to the **repo root**, which is not gitignored — it nearly got committed. Pass a path under `.playwright-mcp/`, or check `git status` and move the file afterwards.
...
- [x] **Step 7 — Wire auth into the backend.** Done — see the Week 2 section above.
- [ ] **Step 8 — Spec-driven request validation** via `nethttp-middleware`. Three traps: `swagger.Servers = nil`, an `AuthenticationFunc` returning nil (kin-openapi enforces `security` and would 401 every dev-mode request), and replacing the plain-text default error handler. Droppable step with a hand-written fallback — see the plan. **It mounts in `internal/app/router.go` now**, on `apiRouter` after the authenticator. Reuse `api.StrictOptions`'s error shape so validation failures match every other 400.
...
- [ ] **Step 14 — Docs**: ADR-0002, `docs/entra-setup.md`, README/HANDOFF updates. (`CLAUDE.md`'s "No CORS" bullet was corrected early, in step 7's doc pass — it now describes the BFF route handler, not the retired rewrite.) Worth folding in: an ADR note that roles are re-read per request rather than cached (plan risk #10), so it reads as a decision.
